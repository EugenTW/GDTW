...
function displaySingleImage(b){const f=document.getElementById("single-photo");f.innerHTML="";var a=b.siEndDate??"N/A",g=b.siTotalVisited??0,d=document.createElement("div");d.classList.add("status-area");d.innerHTML=`Expires on: ${a} | Views: ${g}`;const e=document.createElement("div");e.classList.add("page-url-text");const c=window.location.href;a=new URL(c);e.innerHTML=`Share: ${a.origin+a.pathname}`;e.addEventListener("click",function(){copyToClipboard(c);showCopiedMessage(e)});a=document.createElement("div");
    a.classList.add("photo-url-container");a.appendChild(d);a.appendChild(e);f.appendChild(a);a=document.createElement("div");a.classList.add("photo-wrapper");g=document.createElement("img");d=b.imageUrl.startsWith("http")?b.imageUrl:(new URL(b.imageUrl,window.location.origin)).href;g.src=d;g.alt=b.siName;g.id="photo-img";g.addEventListener("error",function(){this.src="/images/pic_not_found.webp"});a.appendChild(g);if(b.siNsfw===1){const h=document.createElement("div");h.classList.add("nsfw-mask");h.textContent=
        "NSFW - Click to reveal";h.addEventListener("click",()=>{h.classList.add("hidden")});a.appendChild(h)}f.appendChild(a);a=document.createElement("a");a.classList.add("open-link-button");a.innerHTML="\x26#128269;";a.title="\u539f\u59cb\u5c3a\u5bf8 - Full Size";a.href=d;a.target="_blank";a.rel="noopener noreferrer";d=document.createElement("a");d.classList.add("download-link-button");d.innerHTML="\x26#11015;\x26#65039;";d.title="\u4e0b\u8f09\u5716\u7247 - Download";d.addEventListener("click",async h=>
    {h.preventDefault();downloadByUrl(b.imageUrl,b.siName||"download.jpg")});g=document.createElement("div");g.classList.add("url-container");g.appendChild(a);g.appendChild(d);f.appendChild(g);f.classList.remove("hidden")}function showPasswordModal(){const b=document.getElementById("password-modal");b&&b.classList.remove("hidden")}function hidePasswordModal(){const b=document.getElementById("password-modal");b&&b.classList.add("hidden")}
function setupPasswordValidation(b,f,a){const g=document.getElementById("password-form"),d=document.getElementById("password-input"),e=document.getElementById("password-submit");e.disabled=!0;d.addEventListener("blur",function(){const c=d.value.trim();/^[A-Za-z0-9]{4,10}$/.test(c)||c===""||(alert("\u5bc6\u78bc\u683c\u5f0f\u4e0d\u7b26\u5408\u8acb\u8f38\u51654~10\u4f4d\u82f1\u6578\u5b57\nPassword format is incorrect. Please enter 4-10 alphanumeric characters."),d.value="",e.disabled=!0,setTimeout(()=>
{d.focus()},0))});d.addEventListener("input",function(){var c=d.value.trim();c=/^[A-Za-z0-9]{4,10}$/.test(c);e.disabled=!c});g.addEventListener("submit",async function(c){c.preventDefault();c=d.value.trim();if(c!==""){e.disabled=!0;c={code:f,password:c};try{const h=await (await fetchWithRetry(b,c)).json();h.checkPassword?(h.token&&(authToken=h.token),hidePasswordModal(),initPage(a?"/is_api/downloadAlbumImages":"/is_api/downloadSingleImage",a)):(alert("\u5bc6\u78bc\u932f\u8aa4\u8acb\u91cd\u65b0\u8f38\u5165\nIncorrect password, please try again."),
    d.value="",e.disabled=!0,setTimeout(()=>{d.focus()},0))}catch(h){alert("\u7cfb\u7d71\u932f\u8aa4\u8acb\u7a0d\u5f8c\u518d\u8a66\nSystem error, please try again later."),e.disabled=!1}}})}
async function fetchWithRetry(b,f,a=3){let g=0;for(;g<a;)try{const d=await fetch(b,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(f)});if(d.status===429)await new Promise(e=>setTimeout(e,1E3)),g++;else return d}catch(d){break}throw Error("Too many attempts, please try again later.");}function copyToClipboard(b){navigator.clipboard.writeText(b).then(()=>{console.log("Text copied to clipboard:",b)}).catch(f=>{console.error("Failed to copy text: ",f)})}
function showCopiedMessage(){const b=document.createElement("div");b.textContent="\u5df2\u8907\u88fd\u7db2\u5740 - URL Copied!";b.classList.add("toast-message");document.body.appendChild(b);setTimeout(()=>{document.body.removeChild(b)},1500)}function downloadBlob(b,f){b=URL.createObjectURL(b);const a=document.createElement("a");a.href=b;a.download=f;document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(b)}
async function downloadByUrl(b,f){try{const a=await (await fetch(b,{mode:"cors"})).blob();downloadBlob(a,f)}catch(a){console.error("Download failed:",a),alert("\u4e0b\u8f09\u5931\u6557! - Download Failed!")}};